...
def insert_vote():
    ...
    for index, row in vote_data.iterrows():
        ...
        insert_query += f"\t(SELECT (id_candidate) FROM candidate WHERE id_person = (SELECT (id_person) FROM person WHERE person_name = '{person_name}') AND id_party = (SELECT (id_party) FROM party WHERE party_name = '{party_name}')), "
